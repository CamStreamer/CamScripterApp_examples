  ...
  const fetchCameraList = async () => {
    ...
    try {
      ...
      const res = await fetch('/local/camscripter/proxy.cgi', {
        headers: new Headers({
          'x-target-camera-protocol': `${getValues('camera_protocol')}`,
          ...
          'x-target-camera-path':
            '/axis-cgi/param.cgi?action=list&group=root.Image',
        }),
        signal: signal,
      });
      ...
      if (
        fetchId !==
        fetchIdsInProgress.current[fetchIdsInProgress.current.length - 1]
      ) {
        return;
      }
      ...
      fetchIdsInProgress.current = fetchIdsInProgress.current.filter(
        (id) => fetchId !== id
      );
      abortControllers.current = null;
    }
