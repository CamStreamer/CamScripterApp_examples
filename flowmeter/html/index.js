...
function loadSettings() {
  $.get(
    "/local/camscripter/package/settings.cgi?package_name=flowmeter&action=get",
    function (settings) {
...
function resertCounter() {
  $.get(
    "/local/camscripter/proxy/flowmeter/reset_counter.cgi",
    function (data) {
      alarm("Counter reset");
    }
  );
...
function calibrate() {
  ...
  $.get(
    "/local/camscripter/proxy/flowmeter/calibrate.cgi?volume=" + volume,
    function (data) {
      if (data.k_factor) {
        ...
        alarm(
          "Please input the amout of liquid poured through for calibration"
        );
      }
    }
  );
...
function start() {
  $.get("/local/camscripter/proxy/flowmeter/start.cgi", function (data) {
    alarm("Starting!");
  });
}
