...
class CairoPainter extends CairoFrame{
    constructor(res_w, res_h, co_ord, posX, posY, width, height, bg, text, font_color){
        super(0, 0, width, height, bg, text, font_color);
    ...
    async generateImage (co,scale){
        ...
        let access = await this._begin(co,real_scale);
        ...
        co.showCairoImage(this.surface, this._convertor(this.co_ord[0],this.screen_width, this.supPosX, real_scale*this.width),
        this._convertor(this.co_ord[1], this.screen_height, this.supPosY, real_scale*this.height));
        this._destroy(co);
    }
