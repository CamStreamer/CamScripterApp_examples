...
async function createLayout(resolution_w, resolution_h, mm, settings) {
    layout={};
    layout.background = new CairoPainter(resolution_w, resolution_h, settings.coord, settings.pos_x, settings.pos_y, 500, 760, null, null, null);
    console.log(await mm.image('bg'));
    layout.background.setBgImage(await mm.image('bg'), 'fit');
    layout.start_time = new CairoFrame(35, 250, 180, 50, null, '', [1.0, 1.0, 1.0]);
    layout.start_time.setText(settings.start_time, 'A_CENTER');
    layout.current_time = new CairoFrame(260, 250, 180, 50, null, '', [1.0, 1.0, 1.0]);
    layout.current_time.setText('12:20', 'A_CENTER');
    layout.current_beer = new CairoFrame(310, 420, 98, 126, null, '', [1.0, 1.0, 1.0]);
    layout.current_beer.setBgImage(await mm.image('1'), 'fit');
    layout.beer_background = new CairoFrame(90, 420, 98, 126, null, '', [1.0, 1.0, 1.0]);
    layout.beer_background.setBgImage(await mm.image('1'), 'fit');
    layout.beer_count = new CairoFrame(100, 445, 55, 55, null, '', [1.0, 1.0, 1.0]);
...
async function appRun(settings) {
    ...
    if (true || data.ack === 0){
        ...
        layout.current_beer.setBgImage(await mm.image(getBeerIndex(aggregate_volume)), 'fit');
...
class TimeoutWidget extends EventEmitter{
    ...
    async start(){
        ...
        this.timer = setTimeout(()=>{this._one_period();},this.period)
    }
    async _one_period(){
