class CairoFrame {
    ...
    setPadding(){
        if (arguments.length < 1){
            ...
            for (let i = 0; (i < arguments.length) && (i < 4); i++){
                assert (!isNaN(arguments[i]), "Error, argument is not a number!");
    ...
    setBorder(color,width){
        this.border = width !== null ? width : this.border;
    ...
    _doInplaceRotation(co, cairo, scale, rotation){ //TODO: fitování do rámečku při obecném úhlu otočení
        co.cairo('cairo_translate', cairo,  Math.floor(scale *this.width/2), Math.floor(scale * this.height/2));
    ...
    generateOwnImage(co, cairo, ppX, ppY, scale) {
        ...
        if (this.bg_image) {
            ...
            this._doInplaceRotation(co, cairo, scale, this.rotation);
            if (this.bg_type == 'fit') {
